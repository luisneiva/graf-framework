...
public class NodeVariableAssignment extends VariableAssignment {
	...
	/**
	 * This is similar to the certainty attribute in the node
	 * class. The higher it is, the better it seems to make
	 * the assignment represented by this class.
	 * 
	 * This takes into account the score calculated in
	 * Node.calculateCertainty() and does further calculations
	 * applying to the pair of nodes, not just the variable node
	 * as was done in Node.
	 */
	Integer certainty;
	...
	public Boolean verifyEdges(Graph input, Graph before) {
		...
		for(Edge rEdge : ruleNode.getFrom()) {
			...
			for(Edge inEdge : inputNode.getFrom()) {
				...
				//			+ "\n   "+rEdge.getLabel()+" "+inEdge.getLabel());
			}
			if(!match) {
	...
	public void calculateCertainty(ArrayList<String> likelyNodes, Boolean dealged) {
		if(!ruleNode.getName().equals(originalVariable)) {
			System.out.println("error: considering an assignment that has already been made in NodeVariableAssignment.java");
		}
		...
		if(dealged && ruleNode.getFrom().size() == 0) {
			certainty = ruleNode.getCertainty();
			calculateDealgedCertainty(likelyNodes);

			return;
		}
		else if(dealged && ruleNode.getName().contains("#")) {
			certainty = ruleNode.getCertainty();
			return;
		}
				
		inputNode.resetCertainty();
		inputNode.calculateCertainty(likelyNodes, false);
		//System.out.println(toString() + " " + inputNode.getCertainty());
		Integer inCert = inputNode.getCertainty()/10;

		certainty = inCert + ruleNode.getCertainty();
	}
	
	private void calculateDealgedCertainty(ArrayList<String> likelyNodes) {
		inputNode.resetCertainty();
		inputNode.calculateDealgedCertainty(likelyNodes);
		Integer inCert = inputNode.getCertainty()/10;
