...
public class Rule {
	...
	/**
	 * Forbidden contains nodes that must not exist in the before graph.
	...
	public Rule(Graph before, Graph after) {
		this(before, after, new Graph(), false);
	}
	...
	private Boolean dealged;

	public Rule(Graph before, Graph after, Graph forbidden, Boolean dealged) {
		this.before = before;
		this.after = after;
		this.forbidden = forbidden;
		this.dealged = dealged;
	...
	public Rule(Graph before, Graph after, Graph forbidden) {
		this(before, after, forbidden, false);
	}
	 ...
	 * @return Null if it can't be applied. If it can, then return
	 * the fit (to save time finding it again).
	 */
	private PatternMatch subgraph(Graph input, PatternMatch givenAssignments, PatternMatch forceIgnore) {

		Boolean doFinalTest = before.containsVariableNodes();
		...
		Rule copy = new Rule(new Graph(before), 
				new Graph(), new Graph(forbidden), dealged);

		PatternMatch fit = copy.findPatternMatch(input, forceIgnore, givenAssignments);
		...
		ArrayList<Edge> checkEdge = copy.checkForbiddenEdges(input);
		Boolean check4 = checkEdge.size() == 0;
		if(!check4) {

			/*
			 * Okay, so a forbidden edge was in assignment
			 * but that doesn't mean there's no fit.
			 * Let's try it again, but this time ignoring a
			 * variable assignment that involved a node that
			 * was in the forbidden edge.
			 */

			Boolean combineIgnores = forceIgnore != null;

			ArrayList<NodeVariableAssignment> ignorables = new ArrayList<NodeVariableAssignment>();

			for(Edge e : checkEdge) {

				for(VariableAssignment va : fit) {
					NodeVariableAssignment nva = (NodeVariableAssignment) va;

					if((e.getToName().equals(nva.inputNode.getName())
							|| e.getFromName().equals(nva.inputNode.getName()))) {

						Node ruleNode = new Node(nva.originalVariable);
						Node inputNode = new Node(nva.inputNode.getName());
						NodeVariableAssignment newAssign = new NodeVariableAssignment(ruleNode, inputNode);

						ignorables.add(newAssign);
					}
				}
			}

			/*
			 * At the moment we only ignore these one at a time.
			 * To make this more powerful we could try
			 * ignoring pairs of nvas or groups of any size
			 */
			for(NodeVariableAssignment nva : ignorables) {
				PatternMatch singleAssign = new PatternMatch();
				singleAssign.add(nva);

				if(combineIgnores) {
					singleAssign.addAll(forceIgnore);
					//for(VariableAssignment va : singleAssign)
					//	System.out.println("SUPER IGNORE: " + ((NodeVariableAssignment)va));
				}

				PatternMatch test = subgraph(input, givenAssignments, singleAssign);

				if(test != null) {
					//	System.out.println("Hurray! Ignoring " + nva + " worked");
					return test;
					//} else {
					//	System.out.println("ignoring " +nva + " didn't work");
				}
			}
		...
		/*
		 * If there were variable edges corresponding to
		 * variable nodes and they weren't both assigned
		 * to same-named edge and node then the test has
		 * failed.
		 */
		if(doFinalTest) {
			PatternMatch edgeVars = new PatternMatch();
			PatternMatch nodeVars = new PatternMatch();

			for(VariableAssignment va : fit) {
				NodeVariableAssignment nva = (NodeVariableAssignment) va;
				if(nva.originalVariable.contains("#")) {
					edgeVars.add(nva);
				}
				else {
					nodeVars.add(nva);
			...
			for(VariableAssignment eva : edgeVars) {
				NodeVariableAssignment neva = (NodeVariableAssignment) eva;
				Integer index = neva.originalVariable.indexOf("#");
				String edgeName = neva.originalVariable.substring(0, index);
				
				for(VariableAssignment nva : nodeVars) {
					String nodeName = ((NodeVariableAssignment) nva).originalVariable;
					
					if(nodeName.equals(edgeName)) {
						//System.out.println(eva + " should equal " + nva);
						
						Integer edgeAssignIndex = neva.inputNode.getName().indexOf("#");
						String edgeAssign = neva.inputNode.getName().substring(0, edgeAssignIndex);
						
						String nodeAssign = ((NodeVariableAssignment) nva).inputNode.getName();
						
						//System.out.println(edgeAssign + " =???= " + nodeAssign);
						
						if(!edgeAssign.equals(nodeAssign)) {
	 ...
	 *  @return Null if it's fine, otherwise returns the edge that shouldn't be there.
	 */
	private ArrayList<Edge> checkForbiddenEdges(Graph input) {
	...
	private PatternMatch findPatternMatch(Graph input,
		...
		do {
			...
			{
				NodeVariableAssignment pair = (NodeVariableAssignment) susPairIt.next();
				...
				// If we have a variable edge with the same name as a variable node
				// that we have already assigned, then assign that variable to an edge with
				// the same name as the node.
				Boolean specialCase = false;
				if(var.getName().contains("#")) {

					Integer endIndex = var.getName().indexOf('#');
					String originalName = var.getName().substring(0, endIndex);
					for(VariableAssignment va : result) {
						NodeVariableAssignment nva = (NodeVariableAssignment) va;

						if(originalName.equals(nva.originalVariable)) {

							//System.out.println("we suspect the following are equal");
							//System.out.println("\t-"+pair);
							//System.out.println("\t-"+nva);

							if(possib.getName().contains("#")) {

								endIndex = possib.getName().indexOf("#");
								String originalPossName = possib.getName().substring(0, endIndex);
								String curName = nva.inputNode.getName();

								//curName = nva.originalVariable;

								//System.out.println("...,,,;;;"+originalPossName + "=?=" + curName + " " + originalPossName.equals(nva.inputNode.getName()));

								if(originalPossName.equals(curName)) {
									//System.out.println("---> " + originalPossName + " = " + nva.inputNode.getName());
									specialCase = true;
								}
						...
						for(Edge e : before) {
							if(!e.getFromName().contains("#") && e.getFromName().startsWith("?")) {
								if(e.getFromName().equals(originalName)) {
									//System.out.println("do not attempt to solve " + var.getName() + " yet because there is a same-named variable node still to solve " + pair);
									edgeSkip = true;
									continue mainloop;
								}
							}
							else if(!e.getToName().contains("#") && e.getToName().startsWith("?")) {
								if(e.getToName().equals(originalName)) {
									//System.out.println("do not attempt to solve " + var.getName() + " yet because there is a same-named variable node still to solve " + pair);
									edgeSkip = true;
									continue mainloop;
								}
				...
				if(!specialCase && !pair.verifyEdges(input,before)) {					
					susPairIt.remove();
					continue;
				}
				
				if(!specialCase && possib.getName().contains("#")) {
					//System.out.println("still don't do " + pair);
					susPairIt.remove();
					continue;
				}
			...
			if(loopAgain && progressMade) {
				...
				//for(VariableAssignment va : possibilities)
				//	System.out.println("VA2 " + va + " " 
				//			+ ((NodeVariableAssignment) va).getCertainty());
				//System.out.println("\n");
			}
	 ...
	 */
	private void sortPairs(PatternMatch list, ArrayList<String> likelyNodes) {
		...
		for(VariableAssignment varAssign : list) {
			...
			variable.calculateCertainty(likelyNodes, dealged);
		}
		
		for(VariableAssignment varAssignment : list) {
			NodeVariableAssignment nva = (NodeVariableAssignment) varAssignment;
			nva.calculateCertainty(likelyNodes, dealged);
		}
		...
		do {
			...
			for(int n = 0; n < list.size(); n++) {	
				...
				if(n != 0) {
					if(nva.getCertainty() > value) {
						switchThis = n;
						startAgain = true;
						break;
					}
				}
				value = nva.getCertainty();
			}
			value = null;
		}
		while(startAgain);
	...
	private Boolean compareEdges(Graph input) {
		for(Edge beforeEdge : before) {
			...
			if(!present) {
				input.resetCheck();
				//System.out.println("the edge " + beforeEdge + " was missing");
				return false;
			}
		}
		input.resetCheck();
	...
	public ArrayList<PatternMatch> findAllPatternMatches(Graph input, 
		...
		if(subgraph(input, givenAssignments, null) == null) {
			if(useDealg) {
				realg();
				input = input.realg();
			}
		...
		Rule copy = new Rule(new Graph(before), 
				new Graph(after), new Graph(forbidden), dealged);
	...
	public PatternMatch generatePatternMacth(Graph input, PatternMatch givenAssignments) {
		...
		PatternMatch fit = subgraph(input, givenAssignments, null);
		...
		if(useDealg) {
			...
			Iterator<VariableAssignment> varAssignIt = fit.iterator();
			while(varAssignIt.hasNext()) {
				VariableAssignment va = varAssignIt.next();
				NodeVariableAssignment nva = (NodeVariableAssignment) va;
		...
		if(fit.size() != numVars) {
			System.out.println("error in generatePatternMatch(): " +
					"we found " + fit.size() + " assignments\n" +
					"but we were looking for " + numVars);
		}
	...
	public void applyPatternMatch(PatternMatch pattern) {
		...
		/*
		 * This deals with the unusual case where there is a 
		 * variable edge in the after but not the before and
		 * this edge happens to have the same name as a variable
		 * node in the before (and maybe the after too). We 
		 * name this add-edge after the eponymous node.
		 */
		for(Edge e : after) {
			if(e.getLabel().startsWith("?")) {
				//System.out.println("!" + e);

				for(VariableAssignment va : pattern) {
					NodeVariableAssignment nva = (NodeVariableAssignment) va;

					if(nva.originalVariable.equals(e.getLabel())) {

						e.setLabel(nva.inputNode.getName());
						break;
					}
				}
			}
